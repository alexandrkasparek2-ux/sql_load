...
import { PHASE_LABELS, PHASE_COLORS, PHASE_ICONS, type TrainingPhase } from '../services/phaseDetectionService';
...
export default function Plan() {
  ...
  // ── Fázová výživa ────────────────────────────────────────
  const {
    phaseInfo, detectedPhaseInfo, phaseOverride,
    setPhaseOverride, clearPhaseOverride,
    nextRace, allRaces, saveRace, deleteRace,
  } = useTrainingPhase(userId);

  // Race form state
  const [showRaceForm, setShowRaceForm] = useState(false);
  const [newRaceName, setNewRaceName]   = useState('');
  const [newRaceDate, setNewRaceDate]   = useState('');
  const [newRaceType, setNewRaceType]   = useState<'A' | 'B' | 'C'>('A');
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            color: isActive ? PHASE_COLORS[phase] : T.muted,
                            cursor: 'pointer',
                            display: 'flex',
                            alignItems: 'center',
                            gap: 4,
                ...
                <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
                  ...
                  {showRaceForm && (
                    <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8, marginBottom: 12, padding: '10px 12px', background: T.bg, borderRadius: 10 }}>
                      ...
                      <button
                        type="button"
                        disabled={!newRaceName || !newRaceDate || savingRace}
                        onClick={async () => {
                          ...
                          setNewRaceName(''); setNewRaceDate(''); setNewRaceType('A'); setShowRaceForm(false);
                          setSavingRace(false);
                        }}
                    ...
                    return (
                      <div key={race.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderTop: `1px solid ${T.border}` }}>
                        ...
                        <button
                          ...
                          style={{ fontSize: 16, color: T.muted, background: 'none', border: 'none', cursor: 'pointer', padding: '4px 6px', flexShrink: 0 }}
                        >
                          🗑️
                        </button>
                      </div>
