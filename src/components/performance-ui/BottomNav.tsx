...
export function BottomNav({ active, onChange }: Props) {
  return (
    <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: 75, background: 'rgba(7,7,10,0.95)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(180,200,255,0.08)', display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '0 10px 10px', zIndex: 100 }}>
      {items.map(item => {
        ...
        return (
          <button key={item.id} type="button" onClick={() => onChange(item.id)} style={{ position: 'relative', background: 'transparent', border: 0, color: isActive ? 'var(--brand-primary)' : 'var(--text-muted)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, fontSize: 9, letterSpacing: 1, textTransform: 'uppercase', fontWeight: 700, cursor: 'pointer', minWidth: 48 }}>
            {isActive && <div style={{ position: 'absolute', top: -13, left: '50%', transform: 'translateX(-50%)', width: 24, height: 3, background: 'linear-gradient(90deg,#7C5CFF,#4FE3FF)', borderRadius: '0 0 3px 3px' }} />}
